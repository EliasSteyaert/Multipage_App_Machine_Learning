...
if "model" in st.session_state and st.session_state["model"] is not None:
           ...
           if len(uploaded_test_files) == 1:
               with st.expander("Single File Processing", expanded=True):
                   ...
                   if sample_orientation == "Sample data in rows":
                        ...
                        sample_id_col = st.selectbox("Enter the column name that holds sample IDs:")
    ...
    if 'test_data' in locals():
       ...
       # Align test data to the training feature set
        ...
        labels_table = pd.DataFrame({'Class Name': classes, 'Corresponding labels': labels})
        st.write("Class Name + Corresponding labels:")
        st.write(labels_table)

        # Add an empty column between the two data sections
        empty_column = pd.DataFrame({'': [''] * len(results)})
        
        # Concatenate the results, empty column, and labels_table DataFrames
        combined_df = pd.concat([results, empty_column, labels_table], axis=1)
        
        # Convert DataFrame to CSV
        csv_buffer = io.StringIO()
        # Add the combined DataFrame to the CSV
        combined_df.to_csv(csv_buffer, index=False)
        csv_csv = csv_buffer.getvalue()  # Retrieve CSV content as a string
        
        st.download_button(
            label="Download data and labels as CSV",
            data=csv_csv,
            file_name="results_and_labels.csv",
            mime="text/csv"
        )
        ...
        st.download_button(
            ...
            data=excel_buffer,
            file_name="results_and_labels.xlsx",
