...
if 'data' in locals():
    ...
    if Classification_type == "Logistic Regression (L1)":
        ...
        # Evaluate the model
        accuracy_train = accuracy_score(y_train_resampled, rf_model.predict(X_train_resampled))
        ...
        # Optionally, visualize the Random Forest Feature Importances
        ...
        # Assuming X_train_resampled is your resampled gene expression data and y_train_resampled are your labels
        rf = RandomForestClassifier(n_estimators=100, random_state=42)
        rf.fit(X_train_resampled, y_train_resampled)
    
        # Get feature importances and select the top 50 genes
        feature_importances = rf.feature_importances_
        top_n = 50  # Adjust as needed for readability
        top_gene_indices = np.argsort(feature_importances)[-top_n:]  # indices of top 50 genes
    
        # Retrieve gene names (assuming genes contains your gene names)
        # Assuming X_train_selected contains the gene expression data after feature selection
        top_genes = X_train_selected.columns[top_gene_indices]
    
        top_importances = feature_importances[top_gene_indices]
        ...
        sns.heatmap(
            importance_df.set_index("Gene").T,
            cmap="YlGnBu",
            cbar_kws={'label': 'Feature Importance'}
        )
        plt.title("Top Gene Importances from Random Forest")
        st.pyplot(plt) 
    
        # Assuming X_train had columns originally
        # Manually creating default column names if X_train was a NumPy array
            ...
            for i, estimator in enumerate(rf.estimators_):
                if hasattr(estimator, "feature_importances_"):
                    ...
                    feature_importances_df = pd.DataFrame({
                        ...
                        f"Importance (Tree {i})": feature_importances
                    }).sort_values(by=f"Importance (Tree {i})", ascending=False)
                    all_feature_importances.append(feature_importances_df)
    
            # Merge importance DataFrames for each tree, avoiding duplicate column names
            merged_importances_df = pd.concat(all_feature_importances, axis=1)
    
            # Fix column names to avoid duplication
            merged_importances_df.columns = [f"{col}_{i}" if col == 'Feature' else col for i, col in enumerate(merged_importances_df.columns)]
    
            st.write("Feature Importances by Tree:")
            st.table(merged_importances_df)
        else:
            st.write("Feature importances are not available.")
        ...
        pred_prob = RF.predict_proba(X_test_pca)

        # Training and test accuracy
        st.write("Accuracy on the training set:", RF.score(X_train_resampled, y_train_resampled))
        st.write("Accuracy on the test set:", RF.score(X_test_pca, y_test))
...
# Check if the model and preprocessing steps are available for predictions
#if 'model' in locals():#
#    # Option to upload new patient data for prediction
#    uploaded_test_file = st.file_uploader("Upload CSV file with gene expressions for new patients", type="csv")
#
#    if uploaded_test_file:
#        new_data = pd.read_csv(uploaded_test_file)
#
#        # Select only the gene expression columns used in training
#        selected_features = [col for col in new_data.columns if col in gene_expression_columns]
#        if not selected_features:
#            st.error("No matching gene expression columns found in the uploaded file.")
#        else:
...
#
#            # Display results
#            new_data['Prediction'] = predictions_decoded
#            st.write("Predictions for new patient data:")
#            st.write(new_data[['Prediction']])
#
#            # Display distribution of predictions
#            prediction_counts = new_data['Prediction'].value_counts()
#            st.write("Prediction distribution:")
#            st.bar_chart(prediction_counts)
#else:
#    st.warning("Please complete the training step first to enable predictions.")
#